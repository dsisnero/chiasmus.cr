...
describe Chiasmus::Review do
  ...
  describe ".build_plan" do
    ...
    it "'all' focus includes structural, architecture, security, correctness phases" do
      ...
      (phase_names.any? { |name| name.includes?("structural") || name.includes?("overview") }).should be_true
      (phase_names.any? { |name| name.includes?("architecture") || name.includes?("dead") || name.includes?("layer") }).should be_true
      (phase_names.any? { |name| name.includes?("security") || name.includes?("taint") || name.includes?("data flow") }).should be_true
      (phase_names.any? { |name| name.includes?("correctness") || name.includes?("invariant") || name.includes?("bug") }).should be_true
    end
    ...
    it "'security' focus includes taint-propagation via chiasmus_formalize" do
      ...
      has_formalize = plan.phases.any? { |phase| phase.actions.any? { |action| action.tool == "chiasmus_formalize" } }
      has_formalize.should be_true
      mentions_taint = plan.phases.any? { |phase| phase.actions.any?(&.interpret.downcase.includes?("taint")) }
    ...
    it "includes entry_points in graph actions when provided" do
      ...
      dead_code_action = plan.phases.flat_map(&.actions).find { |action| action.tool == "chiasmus_graph" && action.args["analysis"]?.try(&.as_s?) == "dead-code" }
      dead_code_action.should_not be_nil
      if dead_code_action
        dead_code_action.args["entry_points"]?.try(&.as_a.try(&.map(&.as_s))).should eq ["handleRequest", "main"]
      end
    end
    ...
    it "includes a suggestedTemplates section listing named templates with workflow hints" do
      ...
      plan.suggested_templates.each do |templ|
        templ.template.should_not be_empty
        templ.when.should_not be_empty
        templ.workflow.should_not be_empty
      end
    end
  ...
  describe "delta_against PR review mode" do
    ...
    it "delta phase is absent when delta_against is omitted" do
      plan = Chiasmus::Review.build_plan(files)
      phases = plan.phases
      phases.none? { |phase| phase.phase.includes?("delta") || phase.phase.includes?("0.") }.should be_true
    end
