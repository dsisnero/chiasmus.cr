module Chiasmus
  module Skills
    ...
    STARTER_TEMPLATES.concat([
      SkillTemplate.new(name: "invariant-check", domain: "verification", solver: Solvers::SolverType::Z3, signature: "Verify a function postcondition and find invariant counterexamples", skeleton: "{{SLOT:input_declarations}}\n{{SLOT:result_declarations}}\n{{SLOT:function_body}}\n(assert {{SLOT:precondition}})\n(assert (not {{SLOT:postcondition}}))", slots: [SlotDef.new(name: "input_declarations", description: "Declare function parameter variables", format: "(declare-const x Int)"), SlotDef.new(name: "result_declarations", description: "Declare output variables", format: "(declare-const result Int)"), SlotDef.new(name: "function_body", description: "Define input to output behavior", format: "(assert (= result x))"), SlotDef.new(name: "precondition", description: "Valid input constraint", format: "(>= x 0)"), SlotDef.new(name: "postcondition", description: "Expected result property", format: "(>= result 0)")], normalizations: [Normalization.new(source: "function contracts", transform: "Map pre/postconditions and body to SMT assertions")]),
      SkillTemplate.new(name: "state-machine-deadlock", domain: "verification", solver: Solvers::SolverType::Z3, signature: "Check state-machine transitions for unreachable and deadlocked states", skeleton: "{{SLOT:state_declarations}}\n(declare-const from {{SLOT:state_type}})\n(declare-const to {{SLOT:state_type}})\n(assert (= from {{SLOT:source_state}}))\n(assert (= to {{SLOT:target_state}}))\n(assert (or {{SLOT:transitions}}))", slots: [SlotDef.new(name: "state_declarations", description: "Declare all machine states", format: "(declare-datatypes ((State 0)) (((idle) (done))))"), SlotDef.new(name: "state_type", description: "State enumeration type", format: "State"), SlotDef.new(name: "transitions", description: "Valid transition pairs", format: "(and (= from idle) (= to done))"), SlotDef.new(name: "source_state", description: "Source state", format: "idle"), SlotDef.new(name: "target_state", description: "Target state", format: "done")], normalizations: [Normalization.new(source: "state diagram", transform: "Map nodes and edges to declarations and transitions")]),
      SkillTemplate.new(name: "boundary-condition", domain: "verification", solver: Solvers::SolverType::Z3, signature: "Check numeric boundaries for off-by-one errors and limit violations", skeleton: "{{SLOT:input_declarations}}\n(declare-const result {{SLOT:result_type}})\n(assert (= result {{SLOT:computation}}))\n(assert {{SLOT:domain_constraints}})\n(assert {{SLOT:violation_condition}})", slots: [SlotDef.new(name: "input_declarations", description: "Declare numeric inputs", format: "(declare-const i Int)"), SlotDef.new(name: "result_type", description: "Result type", format: "Int"), SlotDef.new(name: "computation", description: "Result expression", format: "(+ i 1)"), SlotDef.new(name: "domain_constraints", description: "Valid bounds", format: "(>= i 0)"), SlotDef.new(name: "violation_condition", description: "Violation condition", format: "(< result 0)")], normalizations: [Normalization.new(source: "loop with array access", transform: "Map index bounds and arithmetic to SMT constraints")]),
      SkillTemplate.new(name: "association-rule-check", domain: "verification", solver: Solvers::SolverType::Prolog, signature: "Detect operations that should co-occur, such as lock and unlock", skeleton: "{{SLOT:call_facts}}\n{{SLOT:required_pairs}}\nmissing_pair(Func, Expected) :- required_pair(First, Expected), calls(Func, First), \\+ calls(Func, Expected).", slots: [SlotDef.new(name: "call_facts", description: "calls(Function, Operation) facts", format: "calls(handler, lock)."), SlotDef.new(name: "required_pairs", description: "required_pair facts", format: "required_pair(lock, unlock).")], normalizations: [Normalization.new(source: "call graph analysis", transform: "Map calls and paired operations to Prolog facts")]),
      SkillTemplate.new(name: "collective-classification", domain: "analysis", solver: Solvers::SolverType::Prolog, signature: "Propagate labels through a call graph from known seed functions", skeleton: "{{SLOT:call_facts}}\n{{SLOT:seed_labels}}\n{{SLOT:label}}_prop(Func) :- {{SLOT:label}}(Func).\n{{SLOT:label}}_prop(Func) :- calls(Func, Callee), {{SLOT:label}}_prop(Callee).", slots: [SlotDef.new(name: "call_facts", description: "calls(Caller, Callee) facts", format: "calls(handler, db_query)."), SlotDef.new(name: "seed_labels", description: "Known labeled functions", format: "sensitive(db_query)."), SlotDef.new(name: "label", description: "Label predicate name", format: "sensitive")], normalizations: [Normalization.new(source: "call graph annotations", transform: "Use annotations as seeds and graph edges for propagation")]),
      SkillTemplate.new(name: "taint-propagation", domain: "analysis", solver: Solvers::SolverType::Prolog, signature: "Trace untrusted data through flows to security-sensitive sinks", skeleton: "{{SLOT:flow_facts}}\n{{SLOT:taint_sources}}\n{{SLOT:sanitizers}}\n{{SLOT:sinks}}\ntainted(X) :- taint_source(X, _).\ntainted(Y) :- flows(X, Y), tainted(X), \\+ sanitize(X).\nviolation(Sink) :- tainted(Sink), sink(Sink).", slots: [SlotDef.new(name: "flow_facts", description: "flows(From, To) facts", format: "flows(user_input, parse)."), SlotDef.new(name: "taint_sources", description: "taint_source facts", format: "taint_source(user_input, injection)."), SlotDef.new(name: "sanitizers", description: "sanitize facts", format: "sanitize(validate)."), SlotDef.new(name: "sinks", description: "sink facts", format: "sink(execute_sql).")], normalizations: [Normalization.new(source: "web route analysis", transform: "Map inputs, flows, sanitizers, and sinks to Prolog facts")]),
    ])
  end
end
