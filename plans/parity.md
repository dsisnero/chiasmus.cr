...
pointer advanced to `55d43881` (upstream v0.1.27–v0.1.28) on 2026-09-07. P63
and P64 are reconciled. The counts therefore describe the last reconciled
baseline and the current vendor surface is tracked by the generated manifests.
...
| P40 ✓ | Durable MCP snapshot-status receipts | Persist an atomic per-snapshot lifecycle receipt (`queued → writing → ready\|failed`) and expose it through `chiasmus_snapshot_status`, so a client can recover after a timed-out or cancelled graph-save response without retrying blind. | Completed: cache lifecycle specs cover ready, failed, and deletion cleanup receipts; MCP transport polls a ready save without re-extraction; existing save→diff coverage remains green, along with focused gates and strict parity checks. |
| P41 ✓ | MCP solve-history wire compatibility | Serialize correction attempts as upstream-compatible ordered `{round, status, error?}` entries instead of leaking internal solver input/result records. | Completed: scripted error→success correction paths prove exact direct and MCP transport history shapes, omit `error` for non-errors, and retain the domain round; focused gates and strict parity checks pass. |
...
| P55 | MCP search partial-read resilience | Preserve successful empty-result searches, but return a typed warning-bearing error when every requested search file is unreadable; retain warnings as structured data rather than interpolating them into an error string. | Red specs cover an unreadable-only request and successful empty corpus. Complete when direct and transport payloads expose the same `{status, error, warnings}` contract, targeted search/MCP specs and strict inventory gates pass, and the feature is committed. |
| P56 | Snapshot write throughput and coalescing | `graph/cache`, `graph/analyses`, and the graph tool: keep extraction cached while bounding JSON serialization/write work, coalescing repeated saves of one snapshot target, and preserving deterministic ready/failed receipts. | Red performance/regression specs reproduce a warm multi-file save within the MCP budget and repeated same-target save requests. Complete when no synchronous cache write blocks the request path, snapshot-status remains accurate, 75-file Chronicle-scale fixture completes under the documented budget, and graph cache/MCP gates pass. |
| P57 | Snapshot continuation and client recovery | Extend the `save_snapshot`/`snapshot_status` contract so a client timing out or disconnecting can identify and retrieve the one in-flight snapshot without restarting extraction or creating duplicate writes. | Red transport specs simulate timeout/disconnect followed by status polling and a later diff. Complete when status has stable identity, terminal receipt, error, and snapshot metadata; retries are idempotent; graph transport and cache lifecycle gates pass. |
| P58 ✓ | Shards MCP runtime freshness | Make the normal release artifact use `shards build chiasmus --release`, invalidate Make builds when Homebrew's tree-sitter runtime changes, and reject stale macOS links before installation. | Completed: a stale 0.26 dylib link failed `verify-runtime`; a Shards release build links the installed 0.27 runtime and passes MCP healthcheck/startup specs. |
| P59 | Graph analysis parity regression corpus | Turn the vendor graph analysis fixtures—summary, callers, insight, diff, map, and snapshot—into a compact Crystal transport corpus that locks result shape, ordering, warnings, and snapshot semantics. | Red fixture/spec groups map to upstream `tests/graph/*.test.ts`. Complete when each supported analysis has direct + transport coverage, intentional differences are ledgered, and source/test drift reports are reviewed. |
| P60 ✓ | Skills and search result-contract audit | Completed 2026-09-08: audited `learning`, `skill-library`, `craft`, `search/engine`, and `mcp-search` contracts. Crystal already covers template listing/querying, metadata persistence and ordering, learning/promotion, semantic-search ranking, no-provider handling, and MCP payload validation. | Focused skills/search/MCP specs and parity manifests pass. The only intentional behavior difference is syntax-aware snippet extraction in `src/chiasmus/search/engine.cr`, which improves on the vendor’s fixed six-line window while retaining hit shape and ranking. |
| P61 ✓ | Solver and formalization recovery semantics | Completed 2026-09-08: reconciled vendor Z3 disposal with Crystal's shared runtime. A disposed facade now rejects new requests idempotently while other sessions continue using the serialized process; solver sessions, bounded correction histories, malformed-output/lint recovery, and formalization feedback remain covered through the crolog/Crig seams. | Red-to-green Z3 facade disposal and cross-session liveness specs, plus focused Z3/session/correction/formalization/validation gates, pass. The inventory records crolog's actor-owned Prolog runtime and Crig completion substitutions. |
| P62 ✓ | LLM and MCP facade conformance sweep | Completed 2026-09-08: audited Crig provider/configuration routing and MCP tool gating, schemas, healthcheck, stdio startup, shutdown, and error envelopes. Signal and transport closure now share an exactly-once cleanup coordinator, preventing double wait-group completion or duplicate store closure. | Red-to-green concurrent shutdown coordination, in-memory transport, stdio-healthcheck, tool-gating, factory/config, and MCP integration gates pass. Vendor-specific adapter objects remain explicit Crig substitutions in the inventory. |
| P63 ✓ | Scheme, Racket, and Common Lisp graph support | Completed 2026-09-08: extension detection; managed grammar lifecycle; S-expression extraction for Scheme, Racket, and Common Lisp; package-aware cross-file calls; namespace lookup; and v4 cache invalidation. The Crystal source-reader fallback intentionally treats reader-dispatch syntax as opaque; ordinary list forms, strings, comments, quotes, and quasiquotes are covered by the regression corpus. | Red-to-green specs cover definitions, imports/exports, lexical scopes, records, package resolution, cache invalidation, map lookup, and MCP graph output. Source/test parity manifests refresh cleanly. |
| P64 ✓ | Prolog batch-session integrity | Completed 2026-09-08: `chiasmus_verify` batch queries now consult one program once, run in order on the crolog-owned isolated worker, retain dynamic state, stop on error, and clean up once. Generated graph and Mermaid reachability rules use `chiasmus_member/2`, avoiding collisions with user `member/2`. | Red-to-green direct solver and MCP specs prove state retention, empty-batch error handling, stop-on-error behavior, and collision-safe reachability. The `PrologBatchInput`, solver, and session API rows are ledgered; source/test parity and solver/graph/MCP gates pass. |

Dependency order: P28 first; then P29 → P30, P31 and P32 independently, P33, P34 and P35 independently, P36, P37, P38, P39, P40, P41, P42, and the subsequent MCP compatibility phases P43–P55. P56 → P57 protect snapshot runtime behavior; P59 verifies graph parity after that work. P63 established the S-expression extraction layer before P59 absorbs its regression corpus. P64 established Prolog batch/session health before P61 re-audits solver recovery semantics. P60, P61, and P62 then proceed in the requested subsystem order: skills/search, solvers/formalization, then LLM adapters/MCP facade. Test-helper rows are closed with their owning feature phase rather than treated as standalone product work.
