...
| P55 ✓ | MCP search partial-read resilience | Completed 2026-09-08: search retains readable-file warnings structurally, returns a typed no-readable-files outcome when every path is skipped, and preserves successful empty-corpus searches without requiring an embedding provider. | Direct and transport specs prove the shared `{status, error, warnings}` payload and successful empty hits; targeted search/MCP and strict parity gates pass. |
