...
| P56 ✓ | Snapshot write throughput and coalescing | Completed 2026-09-08: graph snapshots retain bounded per-target coalescing and ready/failed receipts, but `chiasmus_graph` now queues persistence rather than awaiting JSON encoding/write work on the MCP response path. Clients observe completion through `chiasmus_snapshot_status`. | Red-to-green transport regression holds the snapshot writer at its hook while the graph response completes; cache, diff, receipt, coalescing, and MCP graph gates pass. |
| P57 ✓ | Snapshot continuation and client recovery | Completed 2026-09-08: a named save has a durable queued/writing/ready/failed receipt keyed by cache, repository, and snapshot name. A disconnected caller can reconnect, poll the stable receipt, and diff once ready without restarting extraction; duplicate saves retain target-level coalescing. | Transport regression holds the writer through a simulated disconnect, then proves pending status, ready polling, and a later diff. Cache lifecycle and graph transport gates pass. |
...
| P59 ✓ | Graph analysis parity regression corpus | Completed 2026-09-08: reconciled the vendor graph corpus across summary/callers/callees/reachability/dead-code/cycles/path/impact/facts, insight analyses, diff/snapshot, and map projections. Crystal’s direct and MCP transport suites lock payload shape, ordering, warnings, cache behavior, and snapshot lifecycle. | Reviewed vendor `tests/graph/*.test.ts` against the focused Crystal graph, cache, map, facts, diff, Mermaid, and MCP specs; intentional shard/runtime substitutions remain ledgered and strict source/test drift checks pass. |
